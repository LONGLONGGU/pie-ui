    ...
    <el-scrollbar class="wl-main-scroll">
      ...
      <div class="wl-main-list">
        ...
        >
          ...
          <slot name="table-column-bottom" />
        </el-table>
        <!-- 列表型文件列表 -->
        <ul v-show="!layout.show_list" class="wl-list">
          <li
            ...
            <el-form-item label="导入文件">
              <div class="upload-box">
                <el-upload
                  class="upload"
                  drag
                  accept="png,jpeg,svg,xls,docx,doc,mp4,mp3,zip,rar,xlsx,ppt,pptx"
...
<script>
import submitBtn from '@/components/SubmitBtn' // 导入防抖提交组件
import fadeIn from '@/components/FadeIn' // 导入文件管理器
import fileView from '@/components/FileView' // 导入预览组件
import { arrayToTree, splicParentsUntil, getFileViewType } from '@/utils' // 导入组装树函数、拼接路径函数
import { uploadFiles } from '@/api/zst-server/cloudDlisk'
const guid = '00000000-0000-0000-0000-000000000000'
export default {
  name: 'WlExplorer',
  components: { submitBtn, fileView, fadeIn },
  props: {
    /**
     * 头部更多操作自定义内容
     * 需要包含内容：
  ...
  methods: {
    ...
    filePathChange(item) {
      ...
      const _act_item = this.path.history.find(
    ...
    fileSearch() {
      if (this.file.key !== '') {
        console.log()
        this.$emit('search', this.file.key)
        return
      }
    ...
    updateHistoryData(id, data) {
      const _target = this.path.history.find((i) => i.id === id)
      if (!_target) return
      _target.data = data
    },
    // 显示可移动目录
    ...
    wlDownload() {
      ...
      if (this.file_checked_data.length > 1) {
        this.$message({
          showClose: true,
          message: '请选择一个需要下载的文件',
          type: 'error'
        })
        return
      }
      const file = this.file_checked_data[0]
      if (file.type === 1) {
        this.$message({
          showClose: true,
          message: '请选择文件，而不是文件夹！',
          type: 'error'
        })
        return
      }
    ...
    enterTheLower(row, isFolder) {
      ...
      const _children = this.path.history.find(
    ...
    // 文件上传提交操作
    saveUpload() {
      this.$emit('upload', this.uoload_data, this.fileList, this.closeUpload)
    },
    // 手动上传文件
    handleUpload() {
    ...
    uploadSuccess(res) {
      ...
      const _act = this.path.history.find(
    ...
    pathQuerySearch(queryString, cb) {
      const restaurants = this.selfPathHistory
    ...
    fileTypeIcon(row) {
      ...
      if (['.jpg', '.JPG', '.jpeg', '.JPEG', '.png', '.PNG', '.gif', '.GIF', '.bmp'].includes(_suffix)) {
        // 图片
    ...
    // 预览文件
    previewFile(row) {
      const url = 'http:' + process.env.VUE_APP_BASE_API + process.env.VUE_APP_BASE_FILE_UPLOAD_URL + '/download?fileId=' + row.fileId
      this.previewOptions = url
      this.previewType = getFileViewType(row.suffix)
