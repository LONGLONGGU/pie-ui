  ...
  <div class="personal-panel">
    ...
    >
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button :size="size" type="primary" :loading="updatePwdLoading" @click.native="updatePassword">
          确认</el-button>
      </div>
    </el-dialog>
    ...
    <el-dialog
      title="个人中心"
      width="40%"
      :visible.sync="personCenterDialogVisible"
      :close-on-click-modal="false"
      :modal="false"
    >
      <el-form label-width="80px" :size="size">
        <avatar :image-url="dataForm.avatar" @onremove="onremove" @onsuccess="onsuccess" />
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button :size="size" type="primary" :loading="updatePersonalLoading" @click.native="updatePersonal">
          确认</el-button>
      </div>
    </el-dialog>
  </div>
...
import { logout, download, updatePassword, updatePersonal } from '@/api/admin-server/user'
...
export default {
  ...
  methods: {
      ...
      this.$refs.updatePwdDataForm.validate((valid) => {
        if (valid) {
          ...
          this.$confirm('确认提交吗？', '提示', {}).then(() => {
            ...
            const params = { password: this.updatePwdDataForm.password, newPassword: this.updatePwdDataForm.newPassword }
    ...
    // 获取在线用户数
    countOnlineUser() {
      const pageRequest = { pageNum: 1, pageSize: 10000000 }
